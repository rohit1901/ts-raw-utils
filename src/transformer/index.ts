...
type InferredTypes = {
  ...
  aliasDeclarations: TypeAliasDeclaration[];
};
type KeywordType =
...
const findIntersectionTypes = (code: string) => {
  ...
  const aliasDeclarations = parsedFile.statements.filter(
    isTypeAliasDeclaration,
  );
  ...
  intersectionTypes.push(
    ...aliasDeclarations.filter((t) => isIntersectionTypeNode(t.type)),
  );
  aliasDeclarations
    .filter((t) => !isIntersectionTypeNode(t.type))
    ?.forEach((i) => {
 ...
 */
const createInferredTypes = (
  intersectionType: TypeAliasDeclaration,
  { interfaces, enums, aliasDeclarations }: InferredTypes,
) => {
    ...
    types.forEach((t) => {
      if (isTypeReferenceNode(t)) {
        foundInterfaces.push(
          interfaces.find((i) => i.name.escapedText === t.typeName.getText()),
        );
        foundEnums.push(
          enums.find((e) => e.name.escapedText === t.typeName.getText()),
        );
        foundTypes.push(
          aliasDeclarations.find(
            (ty) => ty.name.escapedText === t.typeName.getText(),
          ),
        );
...
const findDeepestProperties = (
  ...
  if (typeLiterals && isTypeLiteralNode(typeLiterals))
    typeLiterals?.members.forEach((member) => {
      if (isPropertySignature(member)) {
 ...
 * @returns {PropertySignature[]} An array of deepest PropertySignature nodes.
 */
const findDeepestPropertyInInterface = (
  interfaceDeclaration: InterfaceDeclaration,
) => {
 ...
 */
const infer = (
  inferredProperties: PropertySignature[][],
  ...
  foundText.forEach((t) => {
    foundProperties.push(
      ...inferredProperties.find((ip) => ip.map((i) => i.getText() === t)),
    );
  });
 ...
 */
const transformIntersectionType = (
  intersectionType: TypeAliasDeclaration,
  { interfaces, enums, aliasDeclarations }: InferredTypes,
): string | undefined => {
  const inferredTypes = createInferredTypes(intersectionType, {
    interfaces,
    enums,
    aliasDeclarations,
  });
  ...
  inferredTypes.foundInterfaces.forEach((i) => {
    const inferredProperty: PropertySignature[] =
      findDeepestPropertyInInterface(i);
  ...
  inferredTypes.foundTypeLiterals.forEach((i) => {
    const inferredProperty: PropertySignature[] =
      findDeepestPropertyInTypeLiteral(i);
    inferredProperties.push(inferredProperty);
  });
  const foundProperties = uniqWith(
    infer(inferredProperties),
    (arrTh, oth) => arrTh.getText() === oth.getText(),
  );
  if (!foundProperties) return;
  return `${buildTypeParametersSignature(
    intersectionType,
 ...
 */
const buildTypeParametersSignature = (
  intersectionType: TypeAliasDeclaration,
) => {
...
export const transformer = (strings: string) => {
  ...
  const { intersectionTypes, updatedAliasDeclarations, interfaces, enums } =
    findIntersectionTypes(strings);
  transformedTypes.push(
    updatedAliasDeclarations.map((ad) => ad.getText()).join("\n"),
    interfaces.map((i) => i.getText()).join("\n"),
    enums.map((e) => e.getText()).join("\n"),
  );
  intersectionTypes.forEach((i) => {
    transformedTypes.push(
      transformIntersectionType(i, {
        interfaces,
        enums,
        aliasDeclarations: updatedAliasDeclarations,
      }),
    );
  });
